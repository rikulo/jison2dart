...
function parseCode(raw, opts) {
    ...
    if (found && !opts.class) {
        ...
        var i = found.indexOf(' ');
        if (i > 0) {
            if (found.substring(0, i) != 'abstract')
                throw "Unknown %class: " + found;
            opts.abstract = true;
            found = found.substring(i + 1);
        }
        opts.class = found.trim();
    }

    found = parse('%extends', '\n');
    if (found && !opts.extends) opts.extends = found.trim();

    found = parse('%{', '\n%}');
    if (found) opts.code = found;

    found = parse('%case-sensitive', '\n');
    opts.caseSensitive = !found || found.trim() != 'false';
    //console.log(opts);
    return raw;
}
...
function processToDart(parser, parserContent, options) {
    ...
    if (options.caseSensitive == false) opts['case-insensitive'] = true;
    ...
    function jsPerformActionToDart(str) {
        ...
        //Remove break following return to avoid Dead code warning
    ...
    function parserInject() {
        ...
        for (var i in this.symbolsByIndex) {
            ...
            result += '\t\tconst $symbol' + symbol.index + ' = ParserSymbol(\'' + symbol.name + '\', ' + symbol.index + ');\n';
            this.symbols.push('\t\t\t' + symbol.index + ': $symbol' + symbol.index + '');
            this.symbols.push('\t\t\t\'' + symbol.name + '\': $symbol' + symbol.index + '');
        }

        result += '\t\t_symbols = const {\n' + this.symbols.join(',\n') + '\n\t\t};\n\n';

        for (var i in terminals) {
            this.terminals.push('\t\t\t' + i + ': $symbol' + i + '');
        }

        result += '\t\t_terminals = const {\n' + this.terminals.join(',\n') + '\n\t\t};\n\n';
        ...
        for (var i in table) {
            ...
            for (var j in table[i]) {
                ...
                if (item.join) { //is array
                    if (item.length == 1) {
                        action = item[0];
                        items.push('\t\t\t' + j + ': ParserAction(' + actions[action] + ')');
                    } else {
                        action = item[0];
                        state = item[1];
                        items.push('\t\t\t' + j + ': ParserAction(' + actions[action] + ', $table' + state + ')');
                    }
                } else {
                    state = item;
                    items.push('\t\t\t' + j + ': ParserAction(' + actions[action] + ', $table' + state + ')');
            ...
            this.tableInstantiation.push('\t\tvar $table' + i + ' = ParserState(' + i + ')');
        ...
        for (var i in defaultActions) {
            ...
            this.defaultActions.push('\t\t\t' + i + ': ParserAction(' + actions[action] + ', $table' + state + ')');
        }
        ...
        for (var i in productions) {
            ...
            if (production.join) {
                var symbol = production[0],
                    len = production[1];
                this.productions.push('\t\t\t' + i + ': ParserProduction($symbol' + this.symbolsByIndex[symbol].index + ',' + len + ')');
            } else {
                var symbol = production;
                this.productions.push('\t\t\t' + i + ': ParserProduction($symbol' + this.symbolsByIndex[symbol].index + ')');
            }
        }

        result += '\t\t_productions = const {\n' + this.productions.join(',\n') + '\n\t\t};\n';
    ...
    function lexerInject() {
        ...
        for (var i in rules) {
            if (rules[i].startsWith('function(')) {
                ...
                this.rules.push('\t\t' + i + ': RegExp(r\'\'\'' + rules[i].substring(1, rules[i].length - 1) + '\'\'\', caseSensitive: ' +  (opts['case-insensitive'] != true)  + ')');
            }
        }

        result += '\tfinal _rules = <int, RegExp>{\n' + this.rules.join(',\n') + '\n\t};\n\n';

        for (var i in conditions) {
            this.conditions.push('\t\t\'' + i + '\': LexerConditions([ ' + conditions[i].rules.join(',') + '], ' + conditions[i].inclusive + ')');
        }

        return result + '\tstatic const _conditions = const <String, LexerConditions>{\n' + this.conditions.join(',\n') + '\n\t};\n\n';
    }
    ...
    parserRaw = parserRaw
        ...
        .simpleReplace('/*@@ABSTRACT@@*/', options.abstract ? 'abstract ': '')
        .simpleReplace('/*@@EXTENDS@@*/', options.extends || 'DefaultJisonParser')
