...
function processToDart(parser, parserContent, options) {
    ...
    parserRaw = parserRaw
        .simpleReplace('/**/library Jison;/**/', options.library ? 'library ' + options.library + ';' : '')
        .simpleReplace('/**/code;/**/', options.code || '')
        .simpleReplace('/**/class Parser/**/', 'class ' + options.class + (options.extends ? ' extends ' + options.extends : ''))
        .simpleReplace('/**/Parser/**/', options.class)
...
String.prototype.simpleReplace = function (from, to) {
    var i = this.indexOf(from);
    return i >= 0 ? this.substring(0, i) + (to||'') + this.substring(i + from.length): this;
};
