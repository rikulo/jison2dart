...
function processToDart(parser, parserContent, options) {
    ...
    parserRaw = parserRaw
        ...
        .replace('/**/imports Jison;/**/', function () {
            if (options.imports) {
                if (Array.isArray(options.imports)) {
                    return options.imports.map((v) => {
                        if (v.includes(' as ')) {
                            var vv = v.split(' as ');
                            return `import '${vv[0]}' as ${vv[1]};\n`;
                        } else {
                            return `import '${v}';\n`;
                        }
                    }).jion('');
                } else {
                    if (options.imports.includes(' as ')) {
                        var vv = options.imports.split(' as ');
                        return `import '${vv[0]}' as ${vv[1]};\n`;
                    }
                    return `import '${options.imports}';`;
                }
            } else {
                return '';
            }
